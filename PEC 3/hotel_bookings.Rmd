...
mediana_adr <- median(x$adr, na.rm = TRUE)
...
ggplot(data=x, aes(x=adr, after_stat(density))) + 
  ...
  annotate("text", x = media_adr + 70, y = 0.01, label = paste("Media del gasto:", round(media_adr, 2)), colour = "red") +
  labs(
...
```{r}
# Agrupar la columna 'dia' por mes y año
x$mes_anio <-paste0(x$arrival_date_year,'-',x$arrival_date_month)  # Redondear las fechas al primer día del mes

# Crear el gráfico con las fechas agrupadas por mes
ggplot(data=x, aes(x=mes_anio, fill=arrival_date_year)) + 
  geom_bar(stat="count", position="dodge", width=0.8) +  # Agregar barras para contar las llegadas por mes
  theme_light() +
  facet_wrap(~ hotel) +  # Desdoblar por tipo de hotel
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +  # Rotar las etiquetas del eje X para facilitar su lectura
  labs(title="Distribución de las Fechas de Llegada por Año y Tipo de Hotel",
       x="Mes y Año de Llegada",
       y="Frecuencia",
       fill="Año de Llegada")
```
```{r}
...
ggplot(data=x, aes(x=mes_anio, fill=arrival_date_year)) + 
  geom_bar(stat="count", position="dodge", width=0.8) +  # Agregar barras para contar las llegadas por mes
  theme_light() +
  facet_wrap(~ categoria) +  # Desdoblar por tipo de hotel
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +  # Rotar las etiquetas del eje X para facilitar su lectura
  labs(title="Distribución de las Fechas de Llegada por Año y Tipo de Hotel",
       x="Mes y Año de Llegada",
       y="Frecuencia",
